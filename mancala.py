...
Mancala AI Bot
    ...
    modified by: Luigi Sanchez
            Extend to use monte carlo simulation
            extend the ability to battle out two different AI, monte carso simulation vs minimax
'''
...
def mcts_vs_minimax():
    ...
    #initialize MCS AI
    mctsPlayer = mcts.MCTS(P1,board)
    state = mctsPlayer.root
    ...
    while(not state.board.game_over()):
            ...
            hasAction = mctsPlayer.expansion(state, next)

            if hasAction:
                    #Who is the next player
                if next == mctsPlayer.player:
                    #move with this player
                    print "Mcts based AI's turn"
                    state = mctsPlayer.findMove(state, P1, 20)

                else:
                    print "Minimax based AI's turn"
                    # find move
                    move = ai_relative.move(state.board, parallel)
                    # find the appropriate move suggested by Minimax but update the MCTS tree also
                    for validMove in state.actions:
                        state = validMove
                ...
                #check if two players have no action then exit
                if noActions > 1:
                    break
                noActions +=1
    ...
    p1_score = state.board.get_score(P1)
    p2_score = state.board.get_score(P2)

    if p1_score > p2_score:
        print 'Player 1 (Monte Carlo) Wins!'
    elif p1_score < p2_score:
...
if __name__ == '__main__':
    ''' create MCTS object'''
    mctsObj = mcts.MCTS(0,board.Board())
    print("1 : Monte Carlo Ai vs Random player")
    print("2 : Monte Carlo Ai analysis")
    print("3 : Monte Carlo Ai vs Minimax with pruning")
    ...
    if choice == 1:
        ''' Show a single game'''
        mctsObj.prnt = True
        mctsObj.mcts_vs_random(20)
    elif choice == 2:
        ''' Show 10 games of 10 '''
        mctsObj.analysis()
    elif choice == 3:
        ''' monte carlo vs minima'''
        mcts_vs_minimax()
    elif choice == 4:
        ''' watch AI's battle it out'''
        ai_battle()
